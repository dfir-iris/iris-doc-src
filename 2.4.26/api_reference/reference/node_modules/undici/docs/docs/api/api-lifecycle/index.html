...
<html lang="en" class="no-js">
  <head>
      ...
      <meta name="viewport" content="width=device-width,initial-scale=1">
        ...
        <link rel="canonical" href="https://dfir-iris.org/2.4.26/api_reference/reference/node_modules/undici/docs/docs/api/api-lifecycle/">
      
      
        <link rel="prev" href="../WebSocket/">
      
      
        <link rel="next" href="../../best-practices/client-certificate/">
      ...
      <meta name="generator" content="mkdocs-1.6.1, mkdocs-material-9.7.1">
        ...
        <title>Client Lifecycle - Documentation</title>
...
<script>window.plausible = window.plausible || function() { (window.plausible.q = window.plausible.q || []).push(arguments) }</script>
    ...
    <div data-md-component="skip">
        ...
        <a href="#client-lifecycle" class="md-skip">
          Skip to content
        </a>
...
<nav class="md-nav md-nav--secondary" aria-label="Table of contents">
    ...
    <ul class="md-nav__list" data-md-component="toc" data-md-scrollfix>
        ...
        <li class="md-nav__item">
  <a href="#state-transition-overview" class="md-nav__link">
    <span class="md-ellipsis">
        ...
        <li class="md-nav__item">
  <a href="#state-details" class="md-nav__link">
    <span class="md-ellipsis">
    ...
    <nav class="md-nav" aria-label="State details">
  ...
  <a href="#closing" class="md-nav__link">
    <span class="md-ellipsis">
        ...
        closing
          ...
          <li class="md-nav__item">
  <a href="#destroyed" class="md-nav__link">
    <span class="md-ellipsis">
...
<h1 id="client-lifecycle">Client Lifecycle</h1>
<p>An Undici <a href="../Client/">Client</a> can be best described as a state machine. The following list is a summary of the various state transitions the <code>Client</code> will go through in its lifecycle. This document also contains detailed breakdowns of each state.</p>
<blockquote>
...
<h2 id="state-transition-overview">State Transition Overview</h2>
<ul>
<li>A <code>Client</code> begins in the <strong>idle</strong> state with no socket connection and no requests in queue.</li>
...
<li>The <strong>pending</strong> state indicates the underlying socket connection has been successfully established and requests are queueing.</li>
...
<li>The <strong>processing</strong> state initializes to the <strong>processing.running</strong> state.</li>
...
<h3 id="idle">idle</h3>
<p>The <strong>idle</strong> state is the initial state of a <code>Client</code> instance. While an <code>origin</code> is required for instantiating a <code>Client</code> instance, the underlying socket connection will not be established until a request is queued using <a href="../Client/#clientdispatchoptions-handlers"><code>Client.dispatch()</code></a>. By calling <code>Client.dispatch()</code> directly or using one of the multiple implementations (<a href="../Client/#clientconnectoptions-callback"><code>Client.connect()</code></a>, <a href="../Client/#clientpipelineoptions-handler"><code>Client.pipeline()</code></a>, <a href="../Client/#clientrequestoptions-callback"><code>Client.request()</code></a>, <a href="../Client/#clientstreamoptions-factory-callback"><code>Client.stream()</code></a>, and <a href="../Client/#clientupgradeoptions-callback"><code>Client.upgrade()</code></a>), the <code>Client</code> instance will transition from <strong>idle</strong> to <a href="#pending"><strong>pending</strong></a> and then most likely directly to <a href="#processing"><strong>processing</strong></a>.</p>
<p>Calling <a href="../Client/#clientclosecallback"><code>Client.close()</code></a> or <a href="../Client/#clientdestroyerror-callback"><code>Client.destroy()</code></a> transitions directly to the <a href="#destroyed"><strong>destroyed</strong></a> state since the <code>Client</code> instance will have no queued requests in this state.</p>
<h3 id="pending">pending</h3>
<p>The <strong>pending</strong> state signifies a non-processing <code>Client</code>. Upon entering this state, the <code>Client</code> establishes a socket connection and emits the <a href="../Client/#event-connect"><code>'connect'</code></a> event signalling a connection was successfully established with the <code>origin</code> provided during <code>Client</code> instantiation. The internal queue is initially empty, and requests can start queueing.</p>
<p>Calling <a href="../Client/#clientclosecallback"><code>Client.close()</code></a> with queued requests, transitions the <code>Client</code> to the <a href="#processing"><strong>processing</strong></a> state. Without queued requests, it transitions to the <a href="#destroyed"><strong>destroyed</strong></a> state.</p>
<p>Calling <a href="../Client/#clientdestroyerror-callback"><code>Client.destroy()</code></a> transitions directly to the <a href="#destroyed"><strong>destroyed</strong></a> state regardless of existing requests.</p>
<h3 id="processing">processing</h3>
<p>The <strong>processing</strong> state is a state machine within itself. It initializes to the <a href="#running"><strong>processing.running</strong></a> state. The <a href="../Client/#clientdispatchoptions-handlers"><code>Client.dispatch()</code></a>, <a href="../Client/#clientclosecallback"><code>Client.close()</code></a>, and <a href="../Client/#clientdestroyerror-callback"><code>Client.destroy()</code></a> can be called at any time while the <code>Client</code> is in this state. <code>Client.dispatch()</code> will add more requests to the queue while existing requests continue to be processed. <code>Client.close()</code> will transition to the <a href="#closing"><strong>processing.closing</strong></a> state. And <code>Client.destroy()</code> will transition to <a href="#destroyed"><strong>destroyed</strong></a>.</p>
<h4 id="running">running</h4>
<p>In the <strong>processing.running</strong> sub-state, queued requests are being processed in a FIFO order. If a request body requires draining, the <em>needDrain</em> event transitions to the <a href="#busy"><strong>processing.busy</strong></a> sub-state. The <em>close</em> event transitions the Client to the <a href="#closing"><strong>process.closing</strong></a> sub-state. If all queued requests are processed and neither <a href="../Client/#clientclosecallback"><code>Client.close()</code></a> nor <a href="../Client/#clientdestroyerror-callback"><code>Client.destroy()</code></a> are called, then the <a href="#processing"><strong>processing</strong></a> machine will trigger a <em>keepalive</em> event transitioning the <code>Client</code> back to the <a href="#pending"><strong>pending</strong></a> state. During this time, the <code>Client</code> is waiting for the socket connection to timeout, and once it does, it triggers the <em>timeout</em> event and transitions to the <a href="#idle"><strong>idle</strong></a> state.</p>
<h4 id="busy">busy</h4>
<p>This sub-state is only entered when a request body is an instance of <a href="https://nodejs.org/api/stream.html">Stream</a> and requires draining. The <code>Client</code> cannot process additional requests while in this state and must wait until the currently processing request body is completely drained before transitioning back to <a href="#running"><strong>processing.running</strong></a>.</p>
<h4 id="closing">closing</h4>
<p>This sub-state is only entered when a <code>Client</code> instance has queued requests and the <a href="../Client/#clientclosecallback"><code>Client.close()</code></a> method is called. In this state, the <code>Client</code> instance continues to process requests as usual, with the one exception that no additional requests can be queued. Once all of the queued requests are processed, the <code>Client</code> will trigger the <em>done</em> event gracefully entering the <a href="#destroyed"><strong>destroyed</strong></a> state without an error.</p>
<h3 id="destroyed">destroyed</h3>
<p>The <strong>destroyed</strong> state is a final state for the <code>Client</code> instance. Once in this state, a <code>Client</code> is nonfunctional. Calling any other <code>Client</code> methods will result in an <code>ClientDestroyedError</code>.</p>
